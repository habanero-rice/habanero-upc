        ...
        for (j = low; j<=high; j++) {
            ...
            switch (mytype) {
                case workerHc_id: {
                    ...
                    ((paramListWorkerHcInst_t *)inst_param[j])->type = COMP_WORKER;
                }
                break;
                case workerHcComm_id: {
                    ALLOC_PARAM_LIST(inst_param[j], paramListWorkerHcInst_t);
                    ((paramListWorkerHcInst_t *)inst_param[j])->workerId = j; // using "id" for now; TODO: decide if a separate key is needed
                    ((paramListWorkerHcInst_t *)inst_param[j])->type = COMM_WORKER ; // using "id" for now; TODO: decide if a separate key is needed
                }
                break;
                default:
