...
void xeWorkerLoop(ocrPolicyDomain_t * pd, ocrWorker_t * worker) {
    ...
    while(worker->fctPtrs->isRunning(worker)) {
        ...
        if (count != 0) {
            ...
            ocrPolicyCtx_t * alarmContext = orgCtx->clone(orgCtx, PD_MSG_GIVE_ME_WORK);
...
void ceWorkerLoop(ocrPolicyDomain_t * pd, ocrWorker_t * worker) {
    // Build and cache a take context
    ocrPolicyCtx_t * orgCtx = getCurrentWorkerContext();
    ocrPolicyCtx_t * ctx = orgCtx->clone(orgCtx, PD_MSG_MSG_TAKE);
        ...
        if (count != 0) {
            ...
            switch (messageContext->type) {
                ...
                case PD_MSG_GIVE_ME_WORK:
                    {
                        ocrPolicyCtx_t * edtExtractContext = orgCtx->clone(orgCtx, PD_MSG_EDT_TAKE);
                        ...
                        if ( NULL_GUID != taskGuid) {
                            ...
                            ocrPolicyCtx_t * edtInjectContext = orgCtx->clone(orgCtx, PD_MSG_INJECT_EDT);
                            ...
                            ocrPolicyCtx_t * msgBufferContext = orgCtx->clone(orgCtx, PD_MSG_MSG_GIVE);
                ...
                case PD_MSG_TAKE_MY_WORK:
                    {
                        ocrPolicyCtx_t * edtPickupContext = orgCtx->clone(orgCtx, PD_MSG_PICKUP_EDT);
                        ...
                        if ( NULL_GUID != taskGuid ) {
                            ocrPolicyCtx_t * edtStoreContext = orgCtx->clone(orgCtx, PD_MSG_EDT_GIVE);
