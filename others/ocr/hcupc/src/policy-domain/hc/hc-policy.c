...
static void destructOcrPolicyCtxHC ( ocrPolicyCtx_t* self ) {
    //TODO since we're caching policy contexts we do not want to
    //     destroy them. However we should have a way of deallocated
    //     them when tearing down the runtime
    //free(self);
}

static ocrPolicyCtx_t * cloneOcrPolicyCtxHC (ocrPolicyCtx_t * ctxIn, ocrPolicyMsgType_t type) {
    ocrPolicyCtx_t * res = (ocrPolicyCtx_t *)&((ocrPolicyCtxHc_t*) (((ocrPolicyCtxHc_t *)ctxIn)->policyMsgCache))[type];
    return res;
...
static u8 hcCreateEdtTemplate(ocrPolicyDomain_t *self, ocrGuid_t *guid,
                              ocrEdt_t func, u32 paramc, u32 depc, u16 properties, ocrPolicyCtx_t *context) {
    ocrTaskTemplate_t *base = self->taskTemplateFactory->instantiate(self->taskTemplateFactory,
                                                                     func, paramc, depc, properties, NULL);
    *guid = base->guid;
    return 0;
}
