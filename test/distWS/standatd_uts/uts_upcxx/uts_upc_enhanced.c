...
void impl_helpMessage() {
  if (PARALLEL) {
    ...
    printf("   -1  int   poll interval for work stealing\n");
...
inline void vsinit()
{
        srand(GET_THREAD_NUM);
	victims = (int*) malloc(sizeof(int) * (GET_NUM_THREADS));
	victims_bk = (int*) malloc(sizeof(int) * (GET_NUM_THREADS));
...
inline void resetvictim() {
	memcpy(victims, victims_bk, GET_NUM_THREADS*sizeof(int));
}

inline int randvictim(size, rank) {
	int vic;
        do {
               vic = rand()%size;
        } while(vic == rank);
...
int findwork(int k) {
  ...
  int size = GET_NUM_THREADS;
  int rank = GET_THREAD_NUM;
  do {
    workDetected = 0;
    int attempts=1;
    resetvictim();
...
#ifndef NOT_LOAD_AWARE
      if (workProbe >= 2 * k) {
#endif // NOT_LOAD_AWARE
...
void showStats(double elapsedSecs) {
  ...
  if(GET_THREAD_NUM == 0) {
                ...
                printf("time.mu\ttotalSuccessDistSteals\ttotalFailedDistSteals\n");
                printf("%.3f\t%llu\t%llu\n",(elapsedSecs*1000),tsteal,tfail);
