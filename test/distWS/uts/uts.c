...
void uts_showStats(int places, int nPes, double walltime, counter_t nNodes, counter_t nLeaves, counter_t maxDepth) {
  // summarize execution info for machine consumption
  if (verbose == 0) {
    printf("%4d %7.3f %9llu %7.0llu %7.0llu %d %.2f %d %d %1d %f %3d\n",
        nPes*places, walltime, nNodes, (long long)(nNodes/walltime), (long long)((nNodes/walltime)/(nPes*places)), 
  ...
  // summarize execution info for human consumption
    ...
    printf("Wallclock time = %.3f sec, performance = %.0f nodes/sec (%.0f nodes/sec per PE)\n\n",
        walltime, (nNodes / walltime), (nNodes / walltime / (nPes*places)));
  }
}
